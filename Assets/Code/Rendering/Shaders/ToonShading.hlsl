float NStepFunction(float x, int N) {
...
in float ToonRampOffset, in float ToonRampOffsetPoint, in float Ambient, in int steps, out float3 ToonRampOutput, out float3 Direction)
{
    ...
    // set the shader graph node previews
        ...
        half toonRamp = NStepFunction(d + ToonRampOffset, steps);
        ...
        for (int j = 0; j < pixelLightCount; ++j) {
            ...
            // toonramp in a smoothstep
            half toonRampExtra = NStepFunction(d + ToonRampOffsetPoint, steps);
